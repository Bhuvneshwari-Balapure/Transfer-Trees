  ...
  try {
    ...
    if (req.body.type && req.body.targetId) {
      ...
      return res.status(400).json({
        success: false,
        message:
          "Invalid report format. Must include either (type + targetId) for content reports or reportedUserId for user reports.",
      });
    }
  } catch (error) {
...
async function handleContentReport(req, res) {
  ...
  console.log("Processing content report:", {
    type,
    targetId,
    reason,
    additionalInfo,
  ...
  async (req, res) => {
    try {
      ...
      if (action === "temporary_ban" || action === "permanent_ban") {
        const banDuration =
          action === "temporary_ban" ? 7 * 24 * 60 * 60 * 1000 : null; // 7 days for temp ban

        await User.findByIdAndUpdate(report.reportedUser, {
          status: "suspended",
          ...(banDuration && {
            suspendedUntil: new Date(Date.now() + banDuration),
          }),
        });
      }
