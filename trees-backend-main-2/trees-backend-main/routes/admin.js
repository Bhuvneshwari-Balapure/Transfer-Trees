...
router.get("/dashboard/stats", async (req, res) => {
  try {
    ...
    const [totalUsers, activeUsers, totalPosts, totalReels, liveStreams] =
      await Promise.all([
        User.countDocuments(),
        User.countDocuments({
          $or: [
...
router.patch("/users/:id/status", async (req, res) => {
  try {
    ...
    console.log(
      `Admin ${isActive ? "unblocked" : "blocked"} user ${req.params.id}`,
    );
...
router.patch("/users/:id/badges", async (req, res) => {
  try {
    ...
    console.log(
      `Admin updated badges for user ${req.params.id}: ${badges?.join(", ") || "none"}`,
    );
...
router.put("/users/:id/ban", async (req, res) => {
  try {
    ...
    console.log(
      `Admin banned user ${req.params.id}. Reason: ${reason || "No reason provided"}`,
    );
...
router.post("/users/:id/reset-password", async (req, res) => {
  try {
    ...
    const otp = await OTP.createOTP(user.email, "email", "password_reset", {
      ipAddress: req.ip,
      userAgent: req.headers["user-agent"],
    });

    await emailService.sendOTPEmail(user.email, otp.code, "password_reset");
...
router.post("/users", async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      user: { _id: user._id, username, email, name, role },
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get("/debug/collections", async (req, res) => {
  try {
    ...
    const reportsInReports = await mongoose.connection.db
      .collection("reports")
      .countDocuments();
    const reportsInUserReports = await mongoose.connection.db
      .collection("userreports")
      .countDocuments();
    const reportsInContentReports = await mongoose.connection.db
...
router.post("/debug/create-test-report", async (req, res) => {
  try {
    ...
    if (users.length < 2) {
      return res
        .status(400)
        .json({ error: "Need at least 2 users in database" });
    }
...
router.delete("/moderation/content/:id", async (req, res) => {
  try {
    ...
    console.log(
      `Admin deleting report: ${req.params.id} - Reason: ${reason || "No reason provided"}`,
    );
...
router.post("/moderation/ban-user/:userId", async (req, res) => {
  try {
    ...
    console.log(
      `Admin banned user: ${user.username} for ${duration || "permanent"} - ${reason}`,
    );
...
router.post("/notifications/send", async (req, res) => {
  try {
    ...
    // Get target users
    const targetUsers = await AdminNotification.getTargetUsers(
      targetAudience,
      specificGroup,
    );
...
router.put("/psa/:id", async (req, res) => {
  try {
    ...
    const psa = await PSA.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
