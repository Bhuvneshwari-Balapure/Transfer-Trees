...
const VIDEOSDK_API_BASE = import.meta.env.VITE_API_BASE_URL?.replace(/\/$/, "") || "";
...
export const getMyActiveStream = async (): Promise<StreamData | null> => {
  try {
    ...
    // Handle 404 - no active stream (this is normal)
    if (response.status === 404) {
      return null;
    }

    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: "Failed to get active stream" }));
      throw new Error(error.error || "Failed to get active stream");
    }

    const data = await response.json();
    
    // Handle case where no active stream exists (200 response but success: false)
    ...
    return {
      ...
      category: data.stream.category,
      thumbnail: data.stream.thumbnail,
      roomId: data.roomId || data.stream.videoSdkRoomId,
      token: data.token,
    };
  } catch (error) {
    // If it's a 404 or "no stream" error, return null (this is normal)
    if (error instanceof Error && (error.message.includes('404') || error.message.includes('No active stream'))) {
      return null;
    }
